...
[\*]{.highlight}These are not a universally agreed definitions.<br> [\*]{.highlight2}Structured means that data is organised in a way that machines can read it. Such as using common identifiers across items. Unstructured means there is no formalised way to relate objects to one another even if they are related.
...
[\*]{.highlight}This can also be a csv or other plain text file.
...
[\*]{.highlight} Analytics exports as csv
...
[\*]{.highlight2} Analytics export as csv.<br> [\*]{.highlight} Analytics export is for user profile as csv. Therefore requires post processing for a specific ThingLink creation
...
I have struggled to find evidence of the usefulness of user feedback.

Everyone asserts it is useful, but provide no tangible evidence as to why or how.

## Why am I sceptical about the value of user feedback? {.center}
...
- Keep track of learning objects using a plain text file(s) i.e. csv files
- Information about user behaviour is available as csv files via export from either Google Analytics or in-built analytics 
- I have no recommendations for feedback other than to be sceptical about it's value due to lack of evidence
